...
const EditOtherinformation = ({ isOpen, onClose, getAlldata ,singleOtherinformation}) => {
  ...
  useEffect(() => {
    if (singleOtherinformation) {
        setotherInformationdata(singleOtherinformation);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${serverUrl}/api/other-information/edit/${singleOtherinformation._id}`,
    ...
    <div className="fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 backdrop-blur-sm transition-opacity duration-300">
      <div className="sidebar relative m-4 w-2/5 max-w-[90%] max-h-[90vh] overflow-y-auto rounded-lg bg-white text-blue-gray-500 shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="text-xl font-normal">Edit Other Information</div>
          <Input
            label={`Title`}
            value={title}
            name="title"
