...
const EditQuote = ({ isOpen, onClose, singleQuote ,getAlldata}) => {
  ...
  useEffect(() => {
    if (singleQuote) {
      ...
      setSelectedTravellers(singleQuote.travellers?.map(t => ({ id: t._id, name: t.name })));
    }
  }, [singleQuote]);
  ...
  const handleSubmit = (e) => {
    ...
    axios.patch(`${serverUrl}/api/quote/edit/${singleQuote._id}`, formData, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    }).then((res) => {
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="flex justify-between m-auto gap-10 mt-5">
            <div className="w-[70%]">
              <div className="flex justify-between items-center m-auto gap-10 mt-5">
                <div className="w-[100%]">
                  <Select
                    options={options}
                    value={options.find((option) => option.value === formState.destination)}
            ...
            <div className="w-[30%] overflow-y-auto">
              ...
              {data?.map((el) => (
                <div className="flex justify-start items-center m-auto gap-10 mt-5" key={el._id}>
                  <input
                    type="checkbox"
