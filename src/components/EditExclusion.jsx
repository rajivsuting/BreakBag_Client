...
const EditExclusion = ({ isOpen, onClose, getAlldata,singleExclusion }) => {
  ...
  useEffect(() => {
    if (singleExclusion) {
        setExclusiondata(singleExclusion);
    }
  }, [singleExclusion]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${serverUrl}/api/exclusion/edit/${singleExclusion._id}`,
        exclusiondata
      );
    ...
    <div className="fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-60 backdrop-blur-sm transition-opacity duration-300">
      <div className="sidebar relative m-4 w-2/5 max-w-[90%] max-h-[90vh] overflow-y-auto rounded-lg bg-white text-blue-gray-500 shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="text-xl font-normal">Edit Exclusion</div>
          <Input
            label={`Title`}
            value={title}
            name="title"
          ...
          <div className="flex justify-center">
            <Button className="bg-main" type="submit" disabled={isLoading}>
              {isLoading ? "Editing Exclusion..." : "Edit Exclusion"}
            </Button>
          </div>
        </form>
      </div>
