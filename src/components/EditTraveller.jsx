...
const EditTraveller = ({ isOpen, onClose, getAlldata, singleTraveller }) => {
  ...
  useEffect(() => {
    if (singleTraveller) {
      setFormData(singleTraveller);
      // setFormData({
      //     agentAssigned: singleTraveller.destination?._id || ""
      //   });
    }
  }, [singleTraveller]);

  useEffect(() => {
    if (singleTraveller && agentAll.length > 0) {
      // Find the option that matches the existing agent ID
      const existingAgent = agentAll.find(
        (agent) => agent._id === singleTraveller.agentAssigned?._id
      );
      console.log("existingAgent",existingAgent)
      if (existingAgent) {
        setSelectedAgent({
          value: existingAgent._id,
          label: existingAgent.name,
        });
      }
    }
  }, [singleTraveller, agentAll]);
  ...
  useEffect(() => {
    ...
    if (selectedAgent) {
      setFormData((prevState) => ({
        ...prevState,
        agentAssigned: selectedAgent.value,
      }));
    }
  }, [selectedAgent]);
...
console.log(formData)
    try {
      ...
      const response = await axios.put(
        `${serverUrl}/api/traveller/edit/${singleTraveller._id}`,
        formData,
        {
          headers: {
      ...
      toast.success("Traveller updated succesfully");
      getAlldata();
      // Clear the form
      setFormData({
        name: "",
        ...
        <div className="">
          <form className="m-auto" onSubmit={handleSubmit}>
            <div className="m-auto mb-5">
              <div className="font-normal text-xl">Edit Traveller</div>
            </div>
            ...
            <div className="w-[90%] flex justify-center items-center text-center mt-5 m-auto">
              <Button className="bg-main" type="submit" disabled={isLoading}>
                {isLoading ? "Editing..." : "Edit Traveller"}
              </Button>
            </div>
          </form>
        </div>
